...
file_put_contents($debugLog, "[personal] " . date('Y-m-d H:i:s') . " POST=" . json_encode($_POST, JSON_UNESCAPED_UNICODE) . " SESSION=" . json_encode([ 'user_id' => $_SESSION['user_id'] ?? null, 'loan_receipt_no' => $_SESSION['loan_receipt_no'] ?? null ], JSON_UNESCAPED_UNICODE) . "\n", FILE_APPEND);
...
if ($income < 0 || $income > 99999999999.99) {
    $_SESSION['error'] = '月收入金额超出范围，请输入合理的金额';
    redirect('loan_apply.php');
}
...
try {
    ...
    file_put_contents($debugLog, "[personal] " . date('Y-m-d H:i:s') . " SUCCESS receipt_no={$receiptNo}\n", FILE_APPEND);
    $_SESSION['loan_receipt_no'] = $receiptNo;
    redirect('loan_status.php');
    
} catch (Exception $e) {
    dbRollback();
    file_put_contents($debugLog, "[personal] " . date('Y-m-d H:i:s') . " ERROR=" . $e->getMessage() . "\n", FILE_APPEND);
