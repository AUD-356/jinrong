...
$investment = dbGetRow("SELECT i.*, p.name as product_name, p.type as product_type 
    ...
    WHERE i.id = ? AND i.user_id = ? AND i.user_type = 'personal'", [$productId, $userId]);
...
$card = dbGetRow("SELECT * FROM cards WHERE user_id = ? AND user_type = 'personal' AND status = 'active' ORDER BY balance DESC LIMIT 1", [$userId]);
