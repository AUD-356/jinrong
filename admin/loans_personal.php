...
if ($action === 'disburse' && $loanId > 0) {
    ...
    try {
        ...
        // 获取用户首张激活银行卡并更新余额
        $userCard = dbGetRow("SELECT * FROM cards WHERE user_id = ? AND user_type = 'personal' AND status = 'active' ORDER BY id ASC LIMIT 1", [$loan['user_id']]);
        if ($userCard) {
        ...
        // 记录操作日志
        $logStmt = getDB()->prepare("INSERT INTO operation_logs (operator_id, operator_type, action, details, target_id, target_type, ip, created_at) VALUES (?, 'admin', 'disburse_loan', '放款个人贷款', ?, 'loan', ?, NOW())");
...
if ($action === 'view' && $loanId > 0) {
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="bi bi-cash-stack me-2"></i>贷款详情</h2>
            </div>
            <a href="loans_personal.php" class="btn btn-outline-secondary">
...
$where = "l.user_type = 'personal'";
$params = [];
if ($search) {
    $where .= " AND (l.receipt_no LIKE ? OR u.username LIKE ? OR u.real_name LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
...
$countSql = "SELECT COUNT(*) FROM loans l LEFT JOIN users u ON l.user_id = u.id WHERE $where";
$total = dbGetOne($countSql, $params);
$totalPages = ceil($total / $pageSize);
$offset = ($page - 1) * $pageSize;

$sql = "SELECT l.*, u.username, u.real_name, lp.name as product_name FROM loans l LEFT JOIN users u ON l.user_id = u.id LEFT JOIN loan_products lp ON l.product_id = lp.id WHERE $where ORDER BY l.created_at DESC LIMIT $pageSize OFFSET $offset";
...
<div class="page-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2><i class="bi bi-cash-stack me-2"></i>个人贷款管理</h2>
            <p class="text-muted mb-0">管理所有个人用户贷款记录</p>
        </div>
    </div>
</div>
...
<div class="card-box">
    <form method="GET" class="mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                <input type="text" class="form-control" name="search" placeholder="搜索编号/用户名/姓名" value="<?php echo htmlspecialchars($search); ?>">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <a href="loans_personal.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-counterclockwise me-1"></i>重置</a>
            </div>
        </div>
    </form>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="loans_personal.php?action=view&id=<?php echo $loan['id']; ?>" class="btn btn-outline-primary btn-sm" title="查看">
                            <i class="bi bi-eye"></i>
                        </a>
                        ...
                        <a href="loans_personal.php?action=disburse&id=<?php echo $loan['id']; ?>" class="btn btn-outline-success btn-sm ms-1" title="放款" onclick="return confirm('确认放款此笔贷款？')">
