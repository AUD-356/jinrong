...
if ($action === 'approve' && $loanId > 0) {
    ...
    if (!$loan) {
        ...
        redirect('loans_audit_enterprise.php');
    }
    ...
    $logStmt = getDB()->prepare("INSERT INTO operation_logs (operator_id, operator_type, action, details, target_id, target_type, ip, created_at) VALUES (?, 'admin', 'approve_loan', '审核通过企业贷款申请', ?, 'loan', ?, NOW())");
...
if ($action === 'reject' && $loanId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($reason)) {
            ...
            redirect('loans_audit_enterprise.php?action=reject&id=' . $loanId);
        }
        ...
        $logStmt->execute([$_SESSION['admin_id'], '拒绝企业贷款申请: ' . $reason, $loanId, $_SERVER['REMOTE_ADDR'] ?? '']);
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="loans_audit_enterprise.php" class="btn btn-outline-secondary">
        ...
        <form method="POST">
            <div class="mb-3">
                ...
                <select class="form-select mb-2" id="rejectReasonSelect" onchange="toggleReason()">
                    ...
                    <option value="企业资质不符合">企业资质不符合</option>
                    <option value="经营状况不达标">经营状况不达标</option>
                    <option value="信用记录不良">信用记录不良</option>
                    <option value="担保或抵押不足">担保或抵押不足</option>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="loans_audit_enterprise.php" class="btn btn-outline-secondary">取消</a>
            </div>
        </form>
    </div>
    <script>
...
if ($action === 'view' && $loanId > 0) {
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <h2><i class="bi bi-bank me-2"></i>贷款申请详情</h2>
            <a href="loans_audit_enterprise.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>返回列表</a>
        </div>
    </div>
    ...
    <div class="card-box">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <label class="form-label text-muted">月营业额</label>
                <div class="form-control-plaintext">￥<?php echo formatMoney($loan['income'] ?? 0); ?></div>
            </div>
            <div class="col-md-6">
                <label class="form-label text-muted">企业经营年限</label>
        ...
        <hr class="my-4">
        <div class="d-flex gap-2">
            <a href="loans_audit_enterprise.php?action=approve&id=<?php echo $loanId; ?>" class="btn btn-success"><i class="bi bi-check-lg me-1"></i>通过申请</a>
            <a href="loans_audit_enterprise.php?action=reject&id=<?php echo $loanId; ?>" class="btn btn-danger"><i class="bi bi-x-lg me-1"></i>拒绝申请</a>
        </div>
        <?php endif; ?>
    </div>
    <?php
...
$countSql = "SELECT COUNT(*) FROM loans WHERE status = 'pending' AND user_type = 'enterprise'";
$total = dbGetOne($countSql);
$totalPages = ceil($total / $pageSize);
$offset = ($page - 1) * $pageSize;

$sql = "SELECT l.*, e.username, e.company_name, lp.name as product_name FROM loans l LEFT JOIN enterprises e ON l.user_id = e.id LEFT JOIN loan_products lp ON l.product_id = lp.id WHERE l.status = 'pending' AND l.user_type = 'enterprise' ORDER BY l.created_at DESC LIMIT $pageSize OFFSET $offset";
$loans = dbGetAll($sql);
...
<div class="page-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2><i class="bi bi-clipboard-check me-2"></i>企业贷款审核</h2>
            <p class="text-muted mb-0">审核企业用户的贷款申请</p>
        </div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            <a href="loans_audit_enterprise.php?action=view&id=<?php echo $loan['id']; ?>" class="btn btn-outline-primary" title="查看"><i class="bi bi-eye"></i></a>
                            <a href="loans_audit_enterprise.php?action=approve&id=<?php echo $loan['id']; ?>" class="btn btn-outline-success" title="通过"><i class="bi bi-check-lg"></i></a>
                            <a href="loans_audit_enterprise.php?action=reject&id=<?php echo $loan['id']; ?>" class="btn btn-outline-danger" title="拒绝"><i class="bi bi-x-lg"></i></a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
