...
if ($action === 'approve' && $cardId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($bankCode)) {
            ...
            redirect('cards_audit_personal.php?action=approve&id=' . $cardId);
        }
        
        $card = dbGetRow("SELECT * FROM cards WHERE id = ?", [$cardId]);
        if (!$card) {
            $_SESSION['error'] = '申请不存在';
            redirect('cards_audit_personal.php');
        }
        ...
        $logStmt->execute([$_SESSION['admin_id'], "审核通过个人开卡申请，卡号: $cardNo", $cardId, $_SERVER['REMOTE_ADDR'] ?? '']);
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="cards_audit_personal.php" class="btn btn-outline-secondary">
    ...
    <div class="card-box">
        <div class="alert alert-info">
            ...
            将为用户 <strong><?php echo htmlspecialchars($card['real_name']); ?></strong> 开户，请选择开户银行。
        </div>
        
        <form method="POST">
            <h5 class="card-title">申请人信息</h5>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="cards_audit_personal.php" class="btn btn-outline-secondary">取消</a>
            </div>
        </form>
    </div>
    <?php
...
if ($action === 'reject' && $cardId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($reason)) {
            ...
            redirect('cards_audit_personal.php?action=reject&id=' . $cardId);
        }
        ...
        $logStmt->execute([$_SESSION['admin_id'], '拒绝个人开卡申请: ' . $reason, $cardId, $_SERVER['REMOTE_ADDR'] ?? '']);
...
if ($action === 'view' && $cardId > 0) {
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="cards_audit_personal.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>返回列表</a>
        </div>
    </div>
    ...
    <div class="card-box">
        ...
        <hr class="my-4">
        <div class="d-flex gap-2">
            <a href="cards_audit_personal.php?action=approve&id=<?php echo $cardId; ?>" class="btn btn-success"><i class="bi bi-check-lg me-1"></i>通过申请</a>
            <a href="cards_audit_personal.php?action=reject&id=<?php echo $cardId; ?>" class="btn btn-danger"><i class="bi bi-x-lg me-1"></i>拒绝申请</a>
        </div>
        <?php endif; ?>
    </div>
    <?php
...
$countSql = "SELECT COUNT(*) FROM cards WHERE status = 'pending' AND user_type = 'personal'";
$total = dbGetOne($countSql);
$totalPages = ceil($total / $pageSize);
$offset = ($page - 1) * $pageSize;

$sql = "SELECT c.*, u.username, u.real_name FROM cards c LEFT JOIN users u ON c.user_id = u.id WHERE c.status = 'pending' AND c.user_type = 'personal' ORDER BY c.created_at DESC LIMIT $pageSize OFFSET $offset";
$cards = dbGetAll($sql);
...
<div class="page-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2><i class="bi bi-file-earmark-plus me-2"></i>个人开卡申请审核</h2>
            <p class="text-muted mb-0">审核个人用户的开卡申请</p>
        </div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            <a href="cards_audit_personal.php?action=view&id=<?php echo $card['id']; ?>" class="btn btn-outline-primary" title="查看"><i class="bi bi-eye"></i></a>
                            <a href="cards_audit_personal.php?action=approve&id=<?php echo $card['id']; ?>" class="btn btn-outline-success" title="通过"><i class="bi bi-check-lg"></i></a>
                            <a href="cards_audit_personal.php?action=reject&id=<?php echo $card['id']; ?>" class="btn btn-outline-danger" title="拒绝"><i class="bi bi-x-lg"></i></a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
