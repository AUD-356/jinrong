...
if ($action === 'freeze' && $cardId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $reasonType = sanitize($_POST['reason_type'] ?? '');
        $reason = sanitize($_POST['reason'] ?? '');
        
        if (empty($reason)) {
            $_SESSION['error'] = '请填写冻结原因';
            redirect('cards_personal.php?action=freeze&id=' . $cardId);
        }
        ...
        $logStmt->execute([$_SESSION['admin_id'], '冻结个人银行卡: ' . $reason, $cardId, $_SERVER['REMOTE_ADDR'] ?? '']);
        
        $_SESSION['success'] = '卡片已冻结';
        redirect('cards_personal.php');
    }
    ?>
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="cards_personal.php" class="btn btn-outline-secondary">
    ...
    <div class="card-box">
        <form method="POST">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="cards_personal.php" class="btn btn-outline-secondary">取消</a>
            </div>
        </form>
    </div>
...
if ($action === 'unfreeze' && $cardId > 0) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($reason)) {
            ...
            redirect('cards_personal.php?action=unfreeze&id=' . $cardId);
        }
        ...
        $logStmt->execute([$_SESSION['admin_id'], '解冻个人银行卡: ' . $reason, $cardId, $_SERVER['REMOTE_ADDR'] ?? '']);
    ...
    <div class="card-box">
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select mb-2" id="reasonType" onchange="toggleReason()">
                    <option value="">请选择原因类型</option>
                    <option value="用户申请解除">用户申请解除</option>
                    <option value="风险排查完成">风险排查完成</option>
                    <option value="证明材料齐全">证明材料齐全</option>
                    <option value="其他">其他</option>
                </select>
            </div>
            <div class="mb-3">
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-success">
                    <i class="bi bi-unlock me-1"></i>确认解冻
                </button>
                <a href="cards_personal.php" class="btn btn-outline-secondary">取消</a>
            </div>
        </form>
...
if ($action === 'view' && $cardId > 0) {
    $card = dbGetRow("SELECT c.*, u.username, u.real_name, u.phone as user_phone FROM cards c LEFT JOIN users u ON c.user_id = u.id WHERE c.id = ?", [$cardId]);
    if (!$card) {
    ...
    <div class="card-box">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <label class="form-label text-muted">卡片类型</label>
                <div class="form-control-plaintext"><?php echo $card['card_type'] === 'debit' ? '借记卡' : '信用卡'; ?></div>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                <label class="form-label text-muted">绑定手机</label>
            ...
            <div class="col-md-6">
                <label class="form-label text-muted">激活时间</label>
                <div class="form-control-plaintext"><?php echo $card['activated_at'] ? formatDate($card['activated_at']) : '未激活'; ?></div>
            </div>
        ...
        <h5 class="card-title mt-4">所属用户</h5>
        <div class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <div class="form-control-plaintext"><?php echo htmlspecialchars($card['real_name']); ?></div>
            </div>
            <div class="col-md-6">
                <label class="form-label text-muted">手机号码</label>
                <div class="form-control-plaintext"><?php echo htmlspecialchars($card['user_phone']); ?></div>
            </div>
        </div>
        
        <?php if ($card['status'] === 'active'): ?>
        <hr class="my-4">
        <a href="cards_personal.php?action=freeze&id=<?php echo $cardId; ?>" class="btn btn-warning">
            <i class="bi bi-snow me-1"></i>冻结卡片
        </a>
        <?php elseif ($card['status'] === 'frozen'): ?>
        <hr class="my-4">
        <a href="cards_personal.php?action=unfreeze&id=<?php echo $cardId; ?>" class="btn btn-success">
            <i class="bi bi-unlock me-1"></i>解冻卡片
        </a>
        <?php endif; ?>
    </div>
    <?php
...
$where = "c.user_type = 'personal'";
$params = [];
if ($search) {
    $where .= " AND (c.card_no LIKE ? OR u.username LIKE ? OR u.real_name LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
...
$countSql = "SELECT COUNT(*) FROM cards c LEFT JOIN users u ON c.user_id = u.id WHERE $where";
$total = dbGetOne($countSql, $params);
$totalPages = ceil($total / $pageSize);
$offset = ($page - 1) * $pageSize;

$sql = "SELECT c.*, u.username, u.real_name FROM cards c LEFT JOIN users u ON c.user_id = u.id WHERE $where ORDER BY c.created_at DESC LIMIT $pageSize OFFSET $offset";
...
<div class="page-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2><i class="bi bi-credit-card-2-front me-2"></i>个人银行卡管理</h2>
            <p class="text-muted mb-0">管理所有个人用户银行卡</p>
        </div>
    </div>
</div>
...
<div class="card-box">
    <form method="GET" class="mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                <input type="text" class="form-control" name="search" placeholder="搜索卡号/用户名/姓名" value="<?php echo htmlspecialchars($search); ?>">
            </div>
            <div class="col-md-3">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($card['real_name'] ?? ''); ?></td>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm">
                            <a href="cards_personal.php?action=view&id=<?php echo $card['id']; ?>" class="btn btn-outline-primary" title="查看">
                                <i class="bi bi-eye"></i>
                            </a>
                            <?php if ($card['status'] === 'active'): ?>
                            <a href="cards_personal.php?action=freeze&id=<?php echo $card['id']; ?>" class="btn btn-outline-warning" title="冻结">
                                <i class="bi bi-snow"></i>
                            </a>
                            <?php elseif ($card['status'] === 'frozen'): ?>
                            <a href="cards_personal.php?action=unfreeze&id=<?php echo $card['id']; ?>" class="btn btn-outline-success" title="解冻">
